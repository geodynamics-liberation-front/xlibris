...
class Database(object):
    ...
    def _get_columns(self, table):
        ...
        return [name for name, col_type 
            in self._tables[table] if '(' not in col_type ]

    def _create_tables(self):
        """Creates all the database tables if necessary"""
        conn = self._get_db()
        cur = conn.cursor()
...
"""
    ...
    def _update(self, table, set_dic, where_dic):
        # The SET clause
        set_clause = 'SET '+(','.join(["%s =?"%k for k in set_dic.keys()]))
        values = set_dic.values()
        # The WHERE clause
        where = 'WHERE '+(' AND '.join(["%s =?"%k for k in where_dic.keys()]))
        values.extend(where_dic.values())

        sql = 'UPDATE %s %s %s' % (table, set_clause, where)
    ...
    def _delete(self, table, where_dic):
        if len(where_dic)>0:
            where = 'WHERE '+(' AND '.join(["%s =?"%k for k in where_dic.keys()]))
            values = tuple(where_dic.values())
        else:
            where = ''
        conn = self._get_db()
        cur = conn.cursor()
