...
def parse_doi_xml(doi):
	...
	journal=Journal()
	journal.title=journal_xml.full_title.string.strip()
	...
	issue=Issue(volume,issue_number,journal)

	for pub_date in issue_xml.findAll('publication_date'):
		media_type=''
		if 'media_type' in pub_date.attrs:
			media_type=pub_date.attrs['media_type']
		pub=Publication(media_type,strng(pub_date.year),strng(pub_date.month),strng(pub_date.day))
	...
	article=Article(doi,title,url,issue)
	for pub_date in article_xml.findAll('publication_date'):
		pub=Publication(pub_date.attrs['media_type'],strng(pub_date.year),strng(pub_date.month),strng(pub_date.day))
...
def flatten_article(article):
	...
	# Article Information
	article_info['doi']=article.doi
	article_info['title']=article.title
	article_info['url']=article.url
	for media_type,pubdate in article.publications.iteritems:
		article_info['article_'+media_type+'_year']=pubdate.year
		article_info['article_'+media_type+'_month']=pubdate.month
		article_info['article_'+media_type+'_day']=pubdate.day
	for ndx, author in enumerate(article.authors):
		article_info[ORDINAL[ndx]+'_author_given_name']=author.given_name
		article_info[ORDINAL[ndx]+'_author_surname']=author.surname

	# Issue Information
	issue=article.issue
	article_info['issue']=issue.issue
	article_info['volume']=issue.volume
	for media_type,pubdate in issue.publications.iteritems:
		article_info['issue_'+media_type+'_year']=pubdate.year
		article_info['issue_'+media_type+'_month']=pubdate.month
		article_info['issue_'+media_type+'_day']=pubdate.day

	# Journal Information
	journal=issue.journal
	article_info['journal_title']=journal.title
	article_info['journal_abbrev']=journal.abbreviation
	for media_type,issn in journal.issn.iteritems:
		article_info[media_type+"issn"]=issn
